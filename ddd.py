...
page = st.sidebar.selectbox('男子校・女子校・共学校を選んでください。',['男子校', '女子校', '共学校', '偏差値連動のテスト'])
...
school_name_kaisei = {'学校名':'開成', '偏差値':67}
school_name_azabu = {'学校名':'麻布', '偏差値':62}
school_name_men_devi = [school_name_tsukukoma, school_name_kaisei, school_name_azabu]
school_name_deviation = []
n = range(len(school_name_men_devi))


if page == '偏差値連動のテスト':
    devi_min, devi_max = st.sidebar.slider(
    '偏差値によって志望校の絞り込みができます。',
    50, 80, (50, 80)
    )
    for _n in n:
        if devi_max > int(school_name_men_devi[_n]['偏差値']) > devi_min:
            school_name_deviation.append(school_name_men_devi[_n]['学校名'])
    st.selectbox('志望校を選択してください。',school_name_deviation)
    button = st.button('グラフの表示')
